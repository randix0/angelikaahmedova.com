...
class M_products extends CI_Model
{
    ...
    public function getItem($getItem = array(), $fetch = false)
    {
        ...
        if ($fetch) {
            ...
            foreach($details_links as $dl) $details_ids[] = (int)$dl['detail_id'];
            if ($details_ids) $details = $this->db->where_in('id',$details_ids)->get('details')->result_array();

            $product['details_ids'] = $details_ids;
            $product['details'] = $details;


            $colors_links = $this->db->where(array('product_id'=>$product['id']))->get('products_colors')->result_array();
            $colors_ids = $colors = array();
            foreach($colors_links as $dl) $colors_ids[] = (int)$dl['color_id'];
            if ($colors_ids) $colors = $this->db->where_in('id',$colors_ids)->get('colors')->result_array();

            $product['colors_ids'] = $colors_ids;
    ...
    public function get($what = '', $id = 0)
    {
        ...
        if ($row && preg_match('(news)', $what) && isset($what2id[$what]) && $what2id[$what]) {
            $row['photos'] = $this->db->where(array($what2id[$what]=>$row['id']))->get($what.'_photos')->result_array();
        }
        return $row;
    }
